        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(String.format(" fetch request %s ", result));
                }
            } else {
                logger.error(String.format("fetch request return error status:%s code:%s", request.getURI(), response.getStatusLine().getStatusCode()));
            }
            // EntityUtils.consume(rsentity);
        } catch (ClientProtocolException e) {
            logger.error(String.format("fetch request error %s msg %s", request.getURI(), e.getMessage()));
            ...
            if (logger.isDebugEnabled())
                logger.debug(String.format("fetch url:%s consume:%s ", request.getURI(), (System.nanoTime() - watch) / 1000));
        }
        return result;
    }
